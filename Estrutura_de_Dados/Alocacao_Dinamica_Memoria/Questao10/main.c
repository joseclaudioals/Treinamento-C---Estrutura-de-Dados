...
void otimizarMemoria(Aluno** listaAluno, int total, int* capacidade);
    ...
    do{
        ...
        switch(opt){
            ...
            case 2:
                ...
                break;
            case 3:
                otimizarMemoria(&listaAluno, total, &capacidade);
                break;
            case 4:
...
void otimizarMemoria(Aluno** listaAluno, int total, int* capacidade){
    if(total==(*capacidade)){
        printf("memoria ja otimizada!\n");
        return;
    }
        else if(total==0 && (*capacidade)==3){
            printf("memoria ja otimizada!\n");
            return;
        }
            else if(total<*capacidade){
                printf("memoria otimizada!\n");
                Aluno* tempLista = realloc(*listaAluno, sizeof(Aluno)* total);
                if(tempLista==NULL){
                    printf("Erro catastrofico! Nao foi possivel realocar memoria\n");
                    free(tempLista);
                    return;
                }
                (*capacidade)=total;
                *listaAluno = tempLista;
            }
}